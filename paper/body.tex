...
In previous research, scientists usually base their model on the well known susceptible-infected-recovered ($SIR$) model. The advanced methodology of SIHR \cite{zhao2012sihr}, the further developed SEIQR model\cite{tanaka2015study}, inspired other research of this kind, therefore we can find some relatively mature research like the one by $Linyuan Lv$ and $Duan-Bing Chen$, which is the initial inspiration of us. The basic idea of their's is to change the continuous SIR model into a discrete-time model to resemble the spreading of news, and introduce four situations of nodes to 
represent the attitude of people towards news in order to describe the probalistic spreading of news. This model is thoroughly discussed in Section 2$$------intentionally left blank--------$$.
...
\begin{itemize}
	\item \textbf{Nodes}: We can introduce a decay effect to resemble the actual memory loss\cite{zhao2011rumor}; we consider how reliable the node is to others; and there's are different types of nodes representing the mass media, celebrities and normal people.
	%这里用mass media and celebrities%
	\item \textbf{Information}:The information itself, or rather the value of it, determines to which degree people tend to believe it. This is discussed in this paper.
	\item \textbf{Links}:A link may be strong or weak, and the speed of information getting around differs. 
\end{itemize}

The topology structure is distinguished in the following aspects.
\begin{itemize}
	\item \textbf{Adjustment of dynamic social network}, decribed in previous work\cite{gerberry2009seiqr}. It represents people making new contacts and lose touch with old friends.
	\item \textbf{The abidance of $Six Degrees of Separation$\cite{milgram1967small}} is missed in previous work during simulation.
	\item \textbf{Structure entropy}, which makes such much difference that the Small World Structure, Random Structure and Regular Structure distinguishes from each other a lot\cite{miritello2011dynamical}. Furthermore, we witness a change of that when taking into account the historical trend of society, which we will discuss in our work.
	\item \textbf{Directed gragh},where the connection between the nodes are not to be simply described as non-directional links. Some links are unidirectional and some are bidirectional, enabling, for example, the description of information flow between celebrities and ordinary person. The celebrities pass information to the ordinary ones but never receive information from them. Data could be collected by crawling the real world network\cite{Kwak:2010:TSN:1772690.1772751}, and we will give it a thorough consideration in this paper. 
\end{itemize}

As mentioned above, despite the previous research, there are essential points neglected such as the involvement of social media $$something here$$, the change of topology structure and the historical explanation of it, the inherent value of information, etc. In this paper however, we weighed the factors against each other, and deliberately neglect some unimportant factors, the memory decay and periodic adjustment of dynamic social network topology, the influence of which is minor compared to that of the spreading information.  
